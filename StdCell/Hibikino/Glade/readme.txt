...
-------------
※20170302までは https://github.com/akita11/MkLSI_Glade にリポジトリをおいていましたが、今後はこちらを更新していきます。
-------------
...
なおGladeの仕様で、環境変数PYTHONPATHで指定するフォルダに、これらのスクリプト(*.py)を置く必要がある。(Windowsならシステム→詳細設定から環境変数を指定する。※パスの区切りは\(￥)ではなく/を使う模様)
...
・Gladeは%HOME%にある.glade.pyを起動時に読み込むので、例えば以下のようにライブラリを読み込むスクリプトを書いておけば、起動時に読み込める。ただしどうもGladeのバグのようで、環境変数%HOME%は参照していないようで、c:/Users/??? (???=ユーザ名。いわゆるHOME)においておけば読み込まれる。(情報提供：muoさん)
-----------
